...
double getFinalPopulation()
    {
        return pv.initialPopulation + (pv.initialPopulation * (pv.birthRate - pv.deathRate) / 1000);
    }
...
double getPopulationOnDate(int day, int month, int year)
    {
        ...
        double popRatio = (double)(getFinalPopulation() / (double)pv.initialPopulation);

        double ratePerDay = 365 * (pow((double)(getFinalPopulation() / (double)pv.initialPopulation), 0.00273972603) - 1); // this float is 1.0/365
        return pv.initialPopulation * pow((1 + ratePerDay / 365.0), totalDays);
    }
